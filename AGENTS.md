...
- Persist `attempt_count` as the server-assigned, contiguous, one-based sequence. Do not accept client-supplied attempt counts or campaign IDs. Preserve the public IDs of retained attempts when editing or removing follow-ups.
...
- In once-off Basic details, show `Scheduled At` directly below Campaign type. Use the saved attempt with `attempt_count = 1` through `Campaign::firstOnceOffSchedule`, shared as `campaign.scheduled_at`; show `--` when absent. Keep this field available on every once-off tab and nested upload details without loading the full schedule outside its own tab.
